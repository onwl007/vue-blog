  ...
  <Card class="comment-list" :class="{'sub-comment-list':child,loading,empty:!list.length}">
    <template v-if="!child || list.length">
      <div class="list-header" slot="header">
        ...
        <div class="sort">
          <a class="sort-type" :class="{activate:latestSort}" @click="handleSort('createAt',-1)">最新排序</a>
          <i class="seprate"></i>
          <a class="sort-type" :class="{activate:hottestSort}" @click="handleSort('ups',-1)">最新排序</a>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      hasNoMore() {
        return this.pagination.total > 0 && this.pagination.cur_page >= this.pagination.total_page && this.pagination >= 1
      },
      listType() {
